...
uint16_t BMP280_TASK(SPI_HandleTypeDef* spi_handle, int bmp280_config_mode, int* spi_done_flag, uint8_t* BMP280_TX_Buffer, uint8_t* BMP280_RX_Buffer, int BMP280_BUFFER_SIZE)
{
    ...
    if(bmp280_config_mode)
	{
        ...
        code_var = BMP280_CONFIG(bmp280_comfig_params, spi_handle, spi_done_flag, BMP280_TX_Buffer, BMP280_RX_Buffer, 4); //Only writing 2 bytes to these config registers
        }
        else
        {
        /* Call the BMP280_READ function */
        code_var = BMP280_READ(spi_handle, spi_done_flag, BMP280_TX_Buffer, BMP280_RX_Buffer, BMP280_BUFFER_SIZE);
	    }
    return code_var;
}
